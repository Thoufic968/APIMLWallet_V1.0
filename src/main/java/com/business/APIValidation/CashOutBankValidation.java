...
public class CashOutBankValidation extends Exception{
    ...
    public void cashOutBankWithEmptyId_COTB_TC_04() throws Exception {
        Object[][] data = dataProvider.cashoutToBank("user_200");
        ValidatableResponse response = com.business.APIResponse.CashOutToBank.cashOutToBankWithInvalidId(data,"");
        ...
        Validation.assertEqualsInt(response.extract().body().jsonPath().get("respCode"), 0,"Response code");
        Validation.assertEquals(response.extract().body().jsonPath().get("respMsg"),"Id is invalid","Response Message");
